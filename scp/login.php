...
if($_POST) {
    ...
    //
    // When running under PHP's built-in dev server (cli-server), relax the
    // check slightly to avoid local session / cookie quirks breaking login.
    $validCsrf = $ost->checkCSRFToken();
    if (!$validCsrf && php_sapi_name() !== 'cli-server') {
